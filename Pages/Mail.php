...
if(isset($_POST['submit'])){
    ...
    //werkt nog niet, hij checkt in php nogmaals of alle inputvelden ingevuld zijn. Als niet, da wordt hij terug gestuurd 
    //if(empty($voornaam)||empty($achternaam)||empty($emailadres))
    //{
      //header("gebruikertoevoegen.php?voornaam=$voornaam&achternaam=$achternaam&emailadres=$emailadres");
    //}
    ...
    if (mail($email,$subject,$message,$headers)){
      $result = DB::insert("INSERT INTO `users` (`naam`, `email`, `role`, `active`, `activationcode`) VALUES (:naam, :email, :rol, :active, :activationcode)", ['naam' => $name, 'email' => $email, 'rol' => $role, 'active' => $active, 'activationcode' => $guid]);
    ...
    ?>
    <META HTTP-EQUIV="Refresh" CONTENT="0; URL=DocentToevoegen.php?name=<?php echo $name;?>">
    <?php
    }
    $stmt->close();
    $conn->close();
